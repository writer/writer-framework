...
function handleDrop(ev: DragEvent) {
	...
	if (!draggedId) {
		// Read the JSON payload from drag data FIRST (before any other checks)
		...
		const initProperties = Object.keys(dragContent).length > 0
			? { content: dragContent as Record<string, string> }
			: undefined;
		createAndInsertComponent(draggedType, component.value.id, undefined, initProperties);
	} else {
