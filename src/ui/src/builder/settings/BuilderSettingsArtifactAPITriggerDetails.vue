<template>
    <div class="BuilderSettingsArtifactAPITriggerDetails">
        ...
        <WdsTabs class="BuilderSettingsArtifactAPITriggerDetails__tabs" v-model="tab" :tabs="tabs" />
        <div v-if="tab === 'builder'" class="BuilderSettingsArtifactAPITriggerDetails__builder">
            <div
            ...
            >
                ...
                <WdsFieldWrapper :label="fieldValue.name ?? fieldKey" :hint="fieldValue.desc" :is-expansible="true">
                    <BuilderFieldsCode
        ...
        <div v-else class="BuilderSettingsArtifactAPITriggerDetails__api">
            <ol class="BuilderSettingsArtifactAPITriggerDetails__steps">
                <li class="BuilderSettingsArtifactAPITriggerDetails__step" id="stepCreate">
                    <p class="BuilderSettingsArtifactAPITriggerDetails__step__hint">
                        Paste the CURL command into your terminal
                    </p>
                    <WdsButton variant="tertiary" size="small" @click="copyCreate">
                        <i class="material-symbols-outlined">{{ isCreateCopied ? 'check' : 'content_copy' }}</i>
                        Copy cURL command
                    </WdsButton>
                </li>
                <li class="BuilderSettingsArtifactAPITriggerDetails__step" id="stepPaste">
                    <p class="BuilderSettingsArtifactAPITriggerDetails__step__hint">
                ...
                <li class="BuilderSettingsArtifactAPITriggerDetails__step" id="stepPoll">
                    <p class="BuilderSettingsArtifactAPITriggerDetails__step__hint">
                        Lastly, paste the poll async into your terminal to retrieve your artifact
                    </p>
                    <WdsButton variant="tertiary" size="small" @click="copyPoll" :disabled="isPollDisabled">
                        <i class="material-symbols-outlined">{{ isPollCopied ? 'check' : 'content_copy' }}</i>
                        Copy poll async
                    </WdsButton>
                </li>
            </ol>
...
const component = computed(() => wf.getComponentById(ssbm.firstSelectedId.value));
...
const baseURL =
    import.meta.env.VITE_WRITER_BASE_URL ??
    window.location.origin + window.location.pathname;
const agentId = import.meta.env.VITE_WRITER_APP_ID ?? '';
const apiKey = import.meta.env.VITE_WRITER_API_KEY ?? '';

const curlCreate = computed(() =>
    `curl --location --request POST '${baseURL}/v1/agents/${agentId}/blueprints/${blueprintId.value}/jobs' \\` +
...
const { copyText: copyCreate, isCopied: isCreateCopied } = useButtonClipboard(curlCreate);

const isPollDisabled = computed(() => {
    const pollUrl = parsedOutput.value?.poll_url;
    return !pollUrl || pollUrl.trim() === '';
});

const curlPoll = computed(() => {
    const pollUrl = parsedOutput.value?.poll_url ?? '/v1/agents/jobs/JOB_ID';
    return `curl --location --request GET '${baseURL}${pollUrl}' \\` +
           `\n--header 'Authorization: Bearer ${apiKey}'`;
});

const { copyText: copyPoll, isCopied: isPollCopied } = useButtonClipboard(curlPoll);
</script>
