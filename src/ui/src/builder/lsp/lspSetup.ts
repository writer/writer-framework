...
// Encapsulate LSP state to avoid module-level variables
function createLSPState() {
	...
	async function initializeVSCodeServicesForLSP(): Promise<boolean> {
		...
		try {
			// Dynamically import VSCode services initialization
			const { initialize: initializeVSCodeServices } = await import(
	 ...
	 */
	async function setupLSP(): Promise<boolean> {
		try {
...
const lspState = createLSPState();

export const setupLSP = lspState.setupLSP;
export const cleanupLSP = lspState.cleanupLSP;
