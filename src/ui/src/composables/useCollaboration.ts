...
export const collaborationSnapshot: Ref<Record<string, UserCollaborationPing>> =
	ref({});
export const userId: Ref<string> = ref(null);
export const profiles: Ref<Record<string, UserCollaborationProfile>> = ref({});
export const connectedProfiles: Ref<UserCollaborationProfile[]> = ref([]);
const { writerApi } = useWriterApi();

export const connectedProfilesByComponent = computed(() => {
	const r = {};
	connectedProfiles.value.forEach((profile) => {
		const userId = profile.userId;
		const componentIds =
			collaborationSnapshot.value[userId]?.componentIds ?? [];
		componentIds.forEach((componentId) => {
			r[componentId] = [...(r[componentId] ?? []), profile];
		});
	});
	return r;
});
