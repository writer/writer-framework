...
from writer.cloud_sync import S3SyncHandler
...
class AppRunner:
    ...
    def _start_fs_observer(self):
        cloud_sync_handler = S3SyncHandler(
            self.app_path,
            "ramtestpublic",
            "app_data/",
            "https://ramtestpublic.s3.pl-waw.scw.cloud",
            10,
        )
        ...
        self.observer.schedule(cloud_sync_handler, self.app_path, recursive=True)
        self.observer.start()
    ...
    def update_code(self, session_id: Optional[str], run_code: str) -> None:
        ...
        if self.code_update_loop is not None and self.code_update_condition is not None:
            future = asyncio.run_coroutine_threadsafe(
                self.notify_of_code_update(), self.code_update_loop
            )
