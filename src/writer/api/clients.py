...
class BaseAPIClient:
    ...
    def __init__(self):
        ...
        if missing_keys := [k for k, v in self.headers.items() if v is None]:
...
class KeyValueAPIClient(BaseAPIClient):
    ...
    def get_value(self, key: str, user_id: Optional[str] = None) -> Any:
        ...
        if response is None:
            logging.error(f"Value for key '{key}' not found.")
            return None
    ...
    def delete_value(self, key: str, user_id: Optional[str] = None):
        ...
        if response is None:
            logging.error(f"Failed to delete value for key: {key}")
            return False
        else:
