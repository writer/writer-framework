...
from writer.shared_blueprint_service import (
    PublishSharedBlueprintRequest,
    SharedBlueprintMetadata,
    get_shared_blueprint_service,
)
...
def get_asgi_app(
    ...
    async def deploy_shared_blueprint(request: Request):
        ...
        org_id = request.headers.get("x-organization-id") or os.getenv("WRITER_ORG_ID", "1") or "1"
        ...
        service = get_shared_blueprint_service()
        publish_request = PublishSharedBlueprintRequest(
            name=blueprint_name,
            description=data["description"],
            components=blueprint_components,
            metadata=metadata,
            snippet_id=published_snippet_id,
        )

        result = service.publish(
            organization_id=org_id,
            user_id=1,  # TODO: Get from auth headers once service is implemented
    ...
    async def create_block_in_library(request: Request):
        ...
        publish_request = PublishSharedBlueprintRequest(
            name=data["name"],
            description=data["description"],
            components=data["blueprint_components"],
            metadata=metadata,
            snippet_id=None,
        )
        ...
        result = service.publish(
            organization_id=org_id,
            user_id=1,  # TODO: Get from auth headers
            request=publish_request,
        )

        return {"snippet_id": result.snippet_id, "version": result.version_number}
    ...
    async def list_blocks_in_library(request: Request):
        ...
        service = get_shared_blueprint_service()
        blueprints = service.list(org_id)

        # TODO: Add service level search filtering once implemented
        ...
        if search_query:
            ...
            blueprints = [
                ...
                or (b.description and search_lower in b.description.lower())
            ]
    ...
    async def get_block_details(snippet_id: str):
        ...
        service = get_shared_blueprint_service()
        try:
            blueprint = service.get(snippet_id)
        except KeyError:
        ...
        return {
            ...
            "latest_version": {
                ...
                "blueprint_components": blueprint.components,
                "description": blueprint.description,
                "metadata": blueprint.metadata.to_dict(),
    ...
    async def get_block_versions(snippet_id: str):
        ...
        service = get_shared_blueprint_service()
        try:
            versions = service.get_versions(snippet_id)
        except KeyError:
            raise HTTPException(status_code=404, detail="Block not found.")
    ...
    async def install_block_from_library(snippet_id: str):
        ...
        blueprint_description = blueprint.description or ""
    ...
    async def create_block_version(snippet_id: str, request: Request):
        ...
        data = await _get_payload_as_json(request)
        org_id: str = request.headers.get("x-organization-id") or os.getenv("WRITER_ORG_ID", "1") or "1"

        service = get_shared_blueprint_service()

        # Try to get existing blueprint for defaults
        try:
            existing = service.get(snippet_id)
            existing_metadata = existing.metadata
            default_components = existing.components
            default_description = existing.description or ""
            default_name = existing.title
        except KeyError:
            raise HTTPException(status_code=404, detail="Block not found")

        # Determine blueprint_components: use provided or fall back to existing
        if "blueprint_components" in data:
            blueprint_components = data["blueprint_components"]
        else:
            blueprint_components = default_components

        # Build metadata, merging with existing if needed
        if "metadata" in data:
            metadata_dict = data["metadata"]
            metadata = SharedBlueprintMetadata(
                state_inputs=metadata_dict.get("state_inputs", metadata_dict.get("stateInputs", existing_metadata.state_inputs)),
                state_outputs=metadata_dict.get("state_outputs", metadata_dict.get("stateOutputs", existing_metadata.state_outputs)),
                vault_keys=metadata_dict.get("vault_keys", metadata_dict.get("vaultKeys", existing_metadata.vault_keys)),
                dependencies=metadata_dict.get("dependencies", existing_metadata.dependencies),
                author=metadata_dict.get("author", existing_metadata.author),
            )
        else:
        ...
        return {"version": result.version_number}
