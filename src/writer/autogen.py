...
from writerai import BadRequestError, Writer
...
def _get_tool_schemas(artificial_id_to_component: Dict[str, Dict]) -> Dict[str, Dict]:
    return {
        "generate_blueprint": {
            "description": """
...
def _construct_tool_calling_kwargs(artificial_id_to_component: Dict[str, Dict]):
    tool_schemas = _get_tool_schemas(artificial_id_to_component)
    return {
...
def _construct_structured_outputs_kwargs(artificial_id_to_component: Dict[str, Dict]):
    ...
    adjusted_schemas = {}
    for name, schema in tool_schemas.items():
        attributes = schema.pop("parameters")
        adjusted_schemas[name] = {**schema, **attributes}
        ...
        "response_format": {
            ...
            "json_schema": {
                "schema": {
                    "type": "object",
                    "additionalProperties": False,
                    "required": list(adjusted_schemas.keys()),
                    "properties": adjusted_schemas
                }
            }
        },
        "model": "palmyra-x5",
...
def _process_link_blocks(arguments: Optional[Dict[str, List[Dict]]], autogen_state: AutogenState) -> Optional[Mapping[str, Mapping]]:
    ...
    if error_messages:
        autogen_state["messages"].extend(error_messages)
        return None
...
def generate_blueprint(description: str, components: Dict[str, Dict], feature_flags: Optional[List[str]] = None, token_header: Optional[str] = None):
    ...
    for i in range(MAX_ITERATIONS):
        ...
        try:
            ...
            description = e.response.json().get("errors", [{}])[0].get("description", "")
            if "Invalid grammar" in description:
                continue
