...
class TestState:
    def test_state_shema_set_dictionary_in_a_state_should_transform_it_in_state_proxy_and_trigger_mutation(
        self,
    ):
        """
        ...
        assert m == {r"+new\.state\.with\.dots": None, r"+new\.state\.with\.dots.test": "test"}
    ...
    def test_replace_dictionary_content_in_a_state_with_schema_should_transform_it_in_state_proxy_and_trigger_mutation(
        self,
    ):
        """
    ...
    def test_remove_then_replace_nested_dictionary_should_trigger_mutation(self):
        ...
        # Assert
        m = _state._state_proxy.get_mutations_as_dict()
        assert m == {"+nested.c": None, "+nested.c.d": 3, "-nested.c.e": None}
    ...
    def test_subscribe_mutation_with_typed_state_should_manage_mutation(self):
        ...
        with writer_fixtures.new_app_context():
            ...
            initial_state = wf.init_state({"counter": 0, "total": 0}, schema=MyState)
    ...
    def test_subscribe_mutation_should_manage_escaping_in_subscription(self):
        ...
        with writer_fixtures.new_app_context():
            ...
            initial_state = wf.init_state({"a.b": 0, "total": 0})
    ...
    def test_state_shema_should_accept_Dict_generic_typing_for_dict(self):
        ...
        with writer_fixtures.new_app_context():
            ...
            # Acts
            initial_state = wf.init_state({"counter": 0, "record": {}}, schema=MyState)
...
class TestWriterState:
    ...
    def test_mail(self) -> None:
        ...
        assert self.base_s.mail[0] == {"type": "pageChange", "payload": "my_page_key"}
        assert self.base_s.mail[1] == {"type": "my_own_mail", "payload": 2}
...
class TestEventDeserialiser:
    ...
    def test_change(self) -> None:
        ev = WriterEvent(type="wf-change", instancePath=self.root_instance_path, payload="44 !@")
        self.ed.transform(ev)
        assert ev.payload == "44 !@"
    ...
    def test_option_change_default(self) -> None:
        ...
        ev_valid = WriterEvent(type="wf-option-change", instancePath=instance_path, payload="a")

        self.ed.transform(ev_valid)
        assert ev_valid.payload == "a"

        ev_invalid = WriterEvent(type="wf-option-change", instancePath=instance_path, payload="d")
    ...
    def test_option_change(self) -> None:
        ...
        ev_valid = WriterEvent(type="wf-option-change", instancePath=instance_path, payload="sp")

        self.ed.transform(ev_valid)
        assert ev_valid.payload == "sp"

        ev_invalid = WriterEvent(type="wf-option-change", instancePath=instance_path, payload="dk")
    ...
    def test_options_change_default(self) -> None:
        ...
        ev_valid = WriterEvent(
            type="wf-options-change", instancePath=instance_path, payload=["a", "b"]
        )
        ...
        ev_invalid = WriterEvent(
            type="wf-options-change", instancePath=instance_path, payload=["a", "d"]
        )
    ...
    def test_date_change(self) -> None:
        ...
        ev_valid = WriterEvent(
            type="wf-date-change", instancePath=self.root_instance_path, payload="2019-11-23"
        )
        self.ed.transform(ev_valid)
    ...
    def test_range_change(self) -> None:
        ev_invalid = WriterEvent(
            type="wf-range-change", instancePath=self.root_instance_path, payload="virus"
        )
        with pytest.raises(RuntimeError):
            self.ed.transform(ev_invalid)

        ev_valid = WriterEvent(
            type="wf-range-change", instancePath=self.root_instance_path, payload=[10, 42]
        )
        self.ed.transform(ev_valid)
    ...
    def test_time_change(self) -> None:
        ev_invalid = WriterEvent(
            type="wf-time-change", instancePath=self.root_instance_path, payload="virus"
        )
        with pytest.raises(RuntimeError):
            self.ed.transform(ev_invalid)

        ev_valid = WriterEvent(
            type="wf-time-change", instancePath=self.root_instance_path, payload="23:59"
        )
        self.ed.transform(ev_valid)
    ...
    def test_dataframe_update(self) -> None:
        ...
        ev = WriterEvent(
            ...
            instancePath=self.root_instance_path,
            payload={"record": {"number": 1, "text": "one", "empty_text": ""}},
        )
        ...
        ev = WriterEvent(
            type="wf-dataframe-update",
            instancePath=self.root_instance_path,
            payload={"record": {"number": "1n", "text": "one"}},
        )
...
class TestStateSerialiser:
    ...
    def test_polars_df(self) -> None:
        d = {"name": "Normal name", "df": pl.read_csv(self.df_path)}
        s = self.sts.serialise(d)
        assert s.get("name") == "Normal name"
        df_durl = s.get("df")
        df_buffer = urllib.request.urlopen(df_durl)
...
class TestSessionManager:
    ...
    def test_session_verifiers(self) -> None:
        ...
        s_valid = self.sm.get_new_session({"testCookie": "yes"}, {"origin": "example.com"}, None)
        assert s_valid is not None
        s_invalid = self.sm.get_new_session({"testCookie": "no"}, {"origin": "example.com"}, None)
        s_invalid = self.sm.get_new_session({"testCookie": "yes"}, {"origin": "example"}, None)
...
class TestEditableDataFrame:
    def test_editable_dataframe_expose_pandas_dataframe_as_df_property(self) -> None:
        df = pandas.DataFrame({"name": ["Alice", "Bob", "Charlie"], "age": [25, 30, 35]})
    ...
    def test_editable_dataframe_should_read_record_as_dict_based_on_record_index_when_dataframe_has_index(
        self,
    ) -> None:
        df = pandas.DataFrame({"name": ["Alice", "Bob", "Charlie"], "age": [25, 30, 35]})
    ...
    def test_editable_dataframe_should_read_record_as_dict_based_on_record_index_when_dataframe_has_multi_index(
        self,
    ) -> None:
        df = pandas.DataFrame(
            {
    ...
    def test_editable_dataframe_should_process_new_record_into_dataframe_with_multiindex(
        self,
    ) -> None:
        df = pandas.DataFrame(
            {
    ...
    def test_editable_dataframe_should_update_existing_record_as_dateframe_with_multiindex(
        self,
    ) -> None:
        df = pandas.DataFrame(
            {
        ...
        # When
        edf.record_update(
            {"record_index": 0, "record": {"name": "Alicia", "age": 25, "city": "Paris"}}
        )

        # Then
        assert edf.df.iloc[0]["age"] == 25

    def test_editable_dataframe_should_remove_existing_record_as_dateframe_with_multiindex(
        self,
    ) -> None:
        df = pandas.DataFrame(
            {
    ...
    def test_editable_dataframe_expose_polar_dataframe_in_df_property(self) -> None:
        df = polars.DataFrame({"name": ["Alice", "Bob", "Charlie"], "age": [25, 30, 35]})

        edf = wf.EditableDataFrame(df)
        assert edf.df is not None
        assert isinstance(edf.df, polars.DataFrame)

    def test_editable_dataframe_should_read_record_from_polar_as_dict_based_on_record_index(
        self,
    ) -> None:
        df = polars.DataFrame({"name": ["Alice", "Bob", "Charlie"], "age": [25, 30, 35]})
        edf = wf.EditableDataFrame(df)
    ...
    def test_editable_dataframe_should_read_record_from_list_of_record_as_dict_based_on_record_index(
        self,
    ) -> None:
        records = [
            {"name": "Alice", "age": 25},
